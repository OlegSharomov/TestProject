...
BEGIN
	...
	SELECT c.id, u.id, p.id, CASE 
		WHEN u.surname = 'Иванов' THEN 500
		WHEN u.surname = 'Петров' THEN 1000
		WHEN u.surname = 'Сергеев' THEN 1500
	END
	FROM #periods AS p
...
SELECT c.name AS 'Организация', CAST(CONCAT(u.surname, ' ', u.name, ' ', u.patronymic) AS VARCHAR(127)) AS 'ФИО', p.first_day_of_month AS 'Дата начала периода', SUM(a.accruals_sum) AS 'Сумма начисления'
FROM #accruals AS a
...
SELECT c.name AS 'Организация', SUM(accruals_sum) AS 'Сумма начисления'
FROM #accruals AS a
LEFT JOIN #companies AS c ON a.id_company = c.id
LEFT JOIN #periods AS p ON a.id_period = p.id
WHERE p.id <= 2
